...
<small>Kredit per imazhin: [Ahmad Awais ⚡️](https://github.com/ahmadawais)</small>

## Prezantimi

### Motivimi

Ky dokument është një fletor për JavaScript që do të hasni shpesh në projektet bashkëkohore dhe kodin më bashkëkohor.

Ky udhëzues nuk ka për qëllim t'ju mësojë JavaScript nga fillimi, por për të ndihmuar zhvilluesit me njohuri themelore të cilët mund të përpiqen të njihen me kodet moderne (ose të themi për të mësuar React për shembull) për shkak të koncepteve të përdorura ne JavaScript .

Përveç kësaj, ndonjëherë do të jap këshilla personale që mund të jenë të diskutueshme, por do të kujdesem për të përmendur se është një rekomandim personal kur e bëj këtë.

> **Shenim:** Shumica e koncepteve të paraqitura këtu vijnë nga një përditësim i gjuhës JavaScript (ES2015, shpesh i quajtur ES6). Ju mund të gjeni veçori të reja të shtuara nga ky përditësim [këtu] (http://es6-features.org); është bere shumë mirë.

### Burimet Plotësuese

Nëse keni veshtiresi të kuptoni një ide, unë ju sugjeroj që të kërkoni përgjigje për burimet e mëposhtme:
...
- [Fletroja Moderne e JavaScript ](#modern-javascript-cheatsheet)
